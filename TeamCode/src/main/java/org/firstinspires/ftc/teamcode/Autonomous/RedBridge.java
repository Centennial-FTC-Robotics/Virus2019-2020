...
public class RedBridge extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(Agobot.drivetrain.goToPosition(new Vector2D(63, -36), startHeading, 0.6)){

        }

        double startVision = Agobot.clock.milliseconds();
        while (Agobot.clock.milliseconds() < startVision + 700) {} // make the robot wait for one second before reading the stones to ensure stable position
...
//        }
        ...
        while(Agobot.drivetrain.goToPosition(new Vector2D(50, yOffset), 90, 0.6)){
            Agobot.intake.runIntake(1);
            Agobot.intake.getLeft().setPower(1);
            Agobot.intake.getRight().setPower(1);
        }

        while(Agobot.drivetrain.goToPosition(new Vector2D(24, yOffset), 90, 0.6)){

            Agobot.intake.runIntake(1);
            Agobot.intake.getLeft().setPower(1);
            Agobot.intake.getRight().setPower(1);
        }
        while(Agobot.drivetrain.goToPosition(new Vector2D(24, yOffset + 8), 90, 0.6)){
...
//        telemetry.update();
        ...
        while(Agobot.drivetrain.goToPosition(new Vector2D(42, yOffset), 0, 0.6,1.3)){

        }

        //go to foundation
        while(Agobot.drivetrain.goToPosition(new Vector2D(42, 48), 0, 0.6)){
