...
public class BlueDepot extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(Agobot.drivetrain.goToPosition(new Vector2D(-63, -52), startHeading, 0.6)){

        }

        double startVision = Agobot.clock.milliseconds();
        while (Agobot.clock.milliseconds() < (startVision + 2500)) {} // make the robot wait for one second before reading the stones to ensure stable position
...
//        }
        ...
        while(Agobot.drivetrain.goToPosition(new Vector2D(-50, yOffset), 270, 0.6)){
        ...
        while(Agobot.drivetrain.goToPosition(new Vector2D(-23.5, yOffset), 270, 0.6)){

            Agobot.intake.runIntake(1);
            Agobot.intake.getLeft().setPower(1);
            Agobot.intake.getRight().setPower(1);
        }
        while(Agobot.drivetrain.goToPosition(new Vector2D(-24, yOffset - 8), 270, 0.6)){
...
//        telemetry.update();
        ...
        while(Agobot.drivetrain.goToPosition(new Vector2D(-39, yOffset), 180, 0.6,1.3, 3)){

        }

        //go to foundation
        while(Agobot.drivetrain.goToPosition(new Vector2D(-39, 48), 180, 0.8, 1.5, 3)){

        }

        //get closer to foundation
        while(Agobot.drivetrain.goToPosition(new Vector2D(-30, 48), 180, 0.6, 1.5)){
            // place skystone
            Agobot.arm.armFlipOut(true);
        }
        ...
        while(Agobot.drivetrain.goToPosition(new Vector2D(-28, 40), 180, 0.6,1.5)){
        ...
        //drag foundation
        while(Agobot.drivetrain.goToPosition(new Vector2D(-40, 40), 180, 0.6,1.5)){

        }

        PIDControllers.headingController.changeConstants(-.5f, -.15f,-.001f, .9f);

        while(Agobot.drivetrain.goToPosition(new Vector2D(-40, 30), 270, 0.8,1.5)){
        ...
        //push against wall
        while(Agobot.drivetrain.goToPosition(new Vector2D(-40, 42), 270, 0.6, 1.5)){
        ...
        while(Agobot.drivetrain.goToPosition(new Vector2D(-37, 0), 270, 0.6)){
